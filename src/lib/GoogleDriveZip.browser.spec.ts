...
import { GoogleDriveStorage } from './backends/GoogleDriveStorage.ts';
import { MiddlewareZipChunk } from './middleware/MiddlewareZipChunk/MiddlewareZipChunk.ts';

const TEST_ROOT = 'stratus-gdrive-zip-stress-test';
const PASSWORD = 'super-secret-test-password-1234';

const hasAccessToken = typeof process !== 'undefined' && process.env.GOOGLE_ACCESS_TOKEN;
...
describe.runIf(hasAccessToken)('Google Drive Storage with Zip Middleware E2E Stress Tests', () => {
	let backend: GoogleDriveStorage;
	let stratus: StratusBase;

	async function logArchiveState(stratusInstance: StratusBase, actionName: string) {
		...
		try {
			...
			await logToHost(`Failed to log archive state: ${e.message || e}`);
		}
	}
	...
	beforeEach(async () => {
		...
		backend.setCredentials({
			...
			expiresAt: Number(process.env.GOOGLE_EXPIRES_AT || Date.now() + 3600000)
		});
	...
	test('Full lifecycle stress test: Create, Read, Update, Delete, List, Sync, Clear Local, Re-sync Recovery', async () => {
		...
		const file1Content = txtEncoder.encode('Hello Google Drive from Zip E2E!');
		...
		// 2. Perform Sync to push everything to Google Drive
		await logToHost('2. Synchronising local changes to Google Drive...');
		...
		await logArchiveState(stratus, 'First sync (creation of files)');
		...
		expect(txtDecoder.decode(read1)).toBe('Hello Google Drive from Zip E2E!');
		...
		await logArchiveState(stratus, 'Second sync (updates and deletions)');
		...
		// 6. Perform a full Sync from Google Drive to pull everything back
		...
		await logArchiveState(freshTempStratus, 'Recovery sync');
		...
		expect(txtDecoder.decode(restored1)).toBe('Updated Hello Google Drive from Zip E2E!');
	...
	test('Multi-chunk rollover and consolidation stress test', async () => {
		...
		await stratus.sync();
		await logArchiveState(stratus, 'Write & sync /file2.bin');

		await stratus.writeFile('/file3.bin', zeroBuffer);
		await stratus.sync();
		await logArchiveState(stratus, 'Write & sync /file3.bin');

		await stratus.writeFile('/file4.bin', zeroBuffer);
		await stratus.sync();
		await logArchiveState(stratus, 'Write & sync /file4.bin');

		await stratus.writeFile('/file5.bin', zeroBuffer);
		await stratus.sync();
		await logArchiveState(stratus, 'Write & sync /file5.bin');
		...
		await logArchiveState(freshTempStratus, 'Recovery sync after consolidation');
	...
	test('Multi-chunk split in single sync stress test', async () => {
		...
		await stratus.sync();
		await logArchiveState(stratus, 'Single sync of 5 files batch');
